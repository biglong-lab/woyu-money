...
// 月度人事費彙總表
export const monthlyHrCosts = pgTable(
  "monthly_hr_costs",
  {
    id: serial("id").primaryKey(),
    ...
    employerEmploymentInsurance: decimal("employer_employment_insurance", {
      precision: 10,
      scale: 2,
    }),
  ...
  (table) => ({
    uniqueYearMonthEmployee: index("monthly_hr_costs_unique_idx").on(
      table.year,
      table.month,
      table.employeeId
    ),
