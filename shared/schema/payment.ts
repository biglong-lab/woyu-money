...
export const paymentItems = pgTable(
  "payment_items",
  {
    ...
    // 帳單時間掌握（避免遲繳）：帳單來的時間 / 法定付款時間 / 最終必繳日（超過開始罰款）
  ...
  (table) => ({
    ...
    statusNotDeletedIdx: index("payment_items_status_not_deleted_idx").on(
      table.status,
      table.isDeleted
    ),
...
export const paymentRecords = pgTable(
  ...
  {
    id: serial("id").primaryKey(),
    itemId: integer("payment_item_id")
      .notNull()
...
export const paymentSchedules = pgTable(
  ...
  (table) => ({
    ...
    overdueCountIdx: index("payment_schedules_overdue_count_idx").on(
      table.isOverdue,
      table.overdueDays
    ),
  })
...
export const budgetItems = pgTable(
  "budget_items",
  {
    ...
    updatedAt: timestamp("updated_at").defaultNow(),
    // ── PR-1: 館別屬性維度 ─────────────────────────
    // single（單館）| shared（群組分攤）| occupancy（佔用驅動）| company（公司級）
    attribution: varchar("attribution", { length: 20 }).default("single"),
    // single 時：屬於哪個館
    targetProjectId: integer("target_project_id").references(() => paymentProjects.id),
    // shared 時：屬於哪個共用組（FK 在 migration 中設定）
    sharedGroupId: integer("shared_group_id"),
    // shared 時：分攤規則 equal | by_rooms | by_revenue | manual
    allocationRule: varchar("allocation_rule", { length: 20 }),
    // occupancy 時：單日成本（PT 700/天、洗滌 150/天）
    occupancyUnitCost: decimal("occupancy_unit_cost", { precision: 10, scale: 2 }),
    // occupancy 時：預估天數
    occupancyEstimatedDays: integer("occupancy_estimated_days"),
  },
  (table) => ({
    ...
    attributionIdx: index("idx_budget_items_attribution").on(table.attribution),
    targetProjectIdx: index("idx_budget_items_target_project_id").on(table.targetProjectId),
    sharedGroupIdx: index("idx_budget_items_shared_group_id").on(table.sharedGroupId),
  })
)
...
export const insertDailyRevenueSchema = createInsertSchema(dailyRevenues)
  .omit({
    id: true,
    createdAt: true,
    updatedAt: true,
