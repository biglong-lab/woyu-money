...
export function getLaborInsuredSalary(monthlySalary: number): number {
  const grade = LABOR_INSURANCE_GRADES.find((g) => monthlySalary >= g.min && monthlySalary <= g.max)
...
export interface InsuranceCalculationInput {
  ...
  hasInsurance?: boolean // 是否投保（false → 全部保費為 0）
}

// 計算勞健保費用
export function calculateInsurance(input: InsuranceCalculationInput): InsuranceCalculationResult {
  const {
    monthlySalary,
    dependentsCount,
  ...
  // 無投保（兼職/約聘）：所有保費 = 0，淨薪 = 月薪
  if (!hasInsurance) {
    return {
      laborInsuredSalary: 0,
      healthInsuredSalary: 0,
      pensionSalary: 0,
      employerLaborInsurance: 0,
      employerHealthInsurance: 0,
      employerPension: 0,
      employerEmploymentInsurance: 0,
      employerAccidentInsurance: 0,
      employerTotal: 0,
      employeeLaborInsurance: 0,
  ...
  const pensionSalary = input.insuredSalary ? input.insuredSalary : getPensionSalary(monthlySalary)

  // === 雇主負擔 ===

  // 勞保費（普通事故）= 投保薪資 x 普通事故費率 x 雇主比例
  const employerLaborInsurance = Math.round(
    laborInsuredSalary * INSURANCE_RATES.ordinaryAccidentRate * INSURANCE_RATES.laborEmployerRatio
  )
  ...
  const employerAccidentInsurance = Math.round(laborInsuredSalary * occupationalAccidentRate)
  ...
  // 勞退（雇主 6%）
  const employerPension = Math.round(pensionSalary * INSURANCE_RATES.pensionEmployerRate)
  ...
  const employeeLaborInsurance = Math.round(
    laborInsuredSalary * INSURANCE_RATES.ordinaryAccidentRate * INSURANCE_RATES.laborEmployeeRatio
  )
  ...
  const employeeTotal =
    employeeLaborInsurance + employeeEmploymentInsurance + employeeHealthInsurance + employeePension
...
// 計算多位員工的月度人事費彙總
export function calculateMonthlyHRCostSummary(employees: InsuranceCalculationInput[]): {
  totalSalary: number
