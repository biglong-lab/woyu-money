  ...
  app.post("/api/document-inbox/:id/re-recognize", requireAuth, async (req, res) => {
    try {
      ...
      if (doc.imagePath.startsWith('/objects/')) {
        ...
        try {
          const objectStorageService = new ObjectStorageService();
          const objectFile = await objectStorageService.getObjectEntityFile(doc.imagePath);
          const [contents] = await objectFile.download();
          imageBuffer = contents;
          
          // 從 metadata 或副檔名取得 mimeType
          const [metadata] = await objectFile.getMetadata();
          const ext = path.extname(doc.imagePath).toLowerCase();
          mimeType = metadata.contentType || (ext === '.png' ? 'image/png' : ext === '.gif' ? 'image/gif' : 'image/jpeg');
        } catch (objError) {
          console.error("Error reading from Object Storage:", objError);
          await db.update(documentInbox)
            .set({ status: 'failed', updatedAt: new Date() })
            .where(eq(documentInbox.id, id));
          return res.status(404).json({ message: "無法從雲端讀取圖片" });
        }
      } else {
        // 本地檔案路徑（相容舊資料）
