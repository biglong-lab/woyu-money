...
- **三條新 tab 列**（沿用 1.3.0 連結式 tab 模式、新增泛用 LinkTabs 元件）：
  ...
  付款排程（月份分攤/逐筆日期）— 相關頁互達不再靠選單來回跳
- **孤兒頁救回**：專案付款統計掛進付款報表 tab 列（原任何選單都到不了）+ Cmd+K 可搜
- **駕駛艙資料檢視閉環**：現況四卡可點擊（收入/成本/淨利→綜合儀表板、待付→帳單看板）
- **欠款雙頁交叉連結**：歷史欠款整理 ↔ 強制執行管理 頁首互相指引（同是舊債、界線說明）
- **清理**：mobileTabItems 死碼移除（指向 3 個不存在路由）；Cron 健康觀測掛進系統管理選單
- **開發環境**：清除本地 DB 12.4 萬筆測試殘留（216k→92k、subcategory 測試 15s timeout 根因）
...
詳見 `docs/changes/2026-07-08-unified-cashflow.md`。Commit `1eb2190`。
**2026-07-08 11:33 GMT 部署**（容器重建、網站/API 驗證正常）。

- **問題**：四套帳本只有 payment_records 進統一財報，強執繳款/欠款還款是真實
  現金流出卻系統性低估；卡請款結算在收入面完全斷鏈
- **現金流量表**：營業活動新增「強制執行繳款」「歷史欠款還款」負項（計入總計）、
  卡請款到帳列 reference 參考（與 PM 收入可能同筆錢兩時點、防雙算不入總計）
- **儀表板 YTD**：兩帳本月度實付併入 expenseActual + breakdown 新分類、
  month-detail 可點開看逐筆（含案件機關/債權人）；駕駛艙同吃此 API 自動受惠
- **現金流預測**：active 強執分期投影未來月支出、缺口分析涵蓋強執
- **測試**：單元 10 + 整合 4 新增；無 migration、唯讀聚合不動原模組
...
**2026-07-06 14:37 GMT 部署**（隨 1.3.4 一併上線）。
