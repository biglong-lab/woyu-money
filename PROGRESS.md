...
- ✅ **1.2.2** — PM 多租戶遷移（6/14 → t_wodao schema）後直連讀錯 schema、同步 20 天新增 0：
  四檔 Pool 加 search_path、部署後 328 筆缺口自動補回；compare 補排除大號/大哉（6 月差距 0.2%）
- ✅ **1.3.0** — 選單資訊架構重整：7 群工具型 → 5 群做事型、駕駛艙升主要功能、四端同步
- ✅ **1.3.1** — 帳單看板「立即處理」原地付款 + 強執已繳月不再投影
- ✅ **1.3.2** — 帳單看板批次處理（勾選多筆一次付清）
- ✅ 進帳收件箱 293 筆/$100 萬歸帳、5 筆測試殘留清除
- ⏸️ 待使用者決定：39 筆 PMS 月度累計 pending 是否整批拒絕歸零（只用於比對、不可入帳）
- ⏸️ 待外部：PMS 2、3 月資料請對方補登
- ✅ **1.3.3** — 便利性循環 9 輪（批次拒絕/批次分帳、四模組匯出 CSV、收據拍照、
  Cmd+K 壞連結修復、E2E 護欄）詳 [docs/changes/2026-07-04-convenience-loop.md](docs/changes/2026-07-04-convenience-loop.md)、**未部署**

**最後更新**: 2026-07-04 · **最新提交**: `a43531f`（1.3.2 已部署）

---

## 🎯 上階段：全系統五階段優化 ✅ 完成（2026-07-03）
