...
export class BatchImportProcessor {
  ...
  async parseFile(fileBuffer: Buffer, fileName: string): Promise<{ records: ImportRecord[] }> {
    ...
    // 驗證每筆記錄
    const validatedRecords = await Promise.all(records.map((record) => this.validateRecord(record)))
   ...
   * 將行數據映射為記錄格式
   */
  private mapRowToRecord(row: Record<string, unknown>): ImportRecord | null {
    // 支援中文和英文欄位名稱
    const itemName = row["項目名稱"] || row["itemName"] || row["Item Name"] || ""
    const amount = this.parseAmount(
      row["金額"] || row["amount"] || row["Amount"] || row["總金額"] || row["totalAmount"]
    )
    const date = this.parseDate(
      row["日期"] || row["date"] || row["Date"] || row["付款日期"] || row["paymentDate"]
    )
    const projectName =
      row["專案"] || row["project"] || row["Project"] || row["專案名稱"] || row["projectName"] || ""
    const categoryName =
      row["分類"] ||
      row["category"] ||
      row["Category"] ||
    ...
    const priority = parseInt(String(row["優先級"] || row["priority"] || row["Priority"] || "2"))
    const paymentMethod = row["付款方式"] || row["paymentMethod"] || row["Payment Method"] || ""
    const paymentStatus =
      row["付款狀態"] || row["paymentStatus"] || row["Payment Status"] || "未付款"
