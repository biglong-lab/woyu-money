...
export async function getProjectsWithStats(): Promise<ProjectStatItem[]> {
  try {
    const projectStats = await db
      .select({
        ...
        totalAmount: sql<string>`COALESCE(SUM(CASE WHEN ${paymentItems.isDeleted} = false AND (${paymentItems.itemType} IS NULL OR ${paymentItems.itemType} != 'income') THEN ${paymentItems.totalAmount}::numeric ELSE 0 END), 0)`,
        paidAmount: sql<string>`COALESCE(SUM(CASE WHEN ${paymentItems.isDeleted} = false AND (${paymentItems.itemType} IS NULL OR ${paymentItems.itemType} != 'income') THEN ${paymentItems.paidAmount}::numeric ELSE 0 END), 0)`,
        unpaidAmount: sql<string>`COALESCE(SUM(CASE WHEN ${paymentItems.isDeleted} = false AND (${paymentItems.itemType} IS NULL OR ${paymentItems.itemType} != 'income') AND ${paymentItems.status} != 'paid' THEN (${paymentItems.totalAmount}::numeric - ${paymentItems.paidAmount}::numeric) ELSE 0 END), 0)`,
        overdueAmount: sql<string>`COALESCE(SUM(CASE WHEN ${paymentItems.isDeleted} = false AND (${paymentItems.itemType} IS NULL OR ${paymentItems.itemType} != 'income') AND ${paymentItems.status} = 'overdue' THEN (${paymentItems.totalAmount}::numeric - ${paymentItems.paidAmount}::numeric) ELSE 0 END), 0)`,
