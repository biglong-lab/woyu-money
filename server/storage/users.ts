...
export async function updateUserLoginAttempts(id: number, attempts: number, lockedUntil?: Date): Promise<void> {
...
export async function updateUserPermissions(id: number, permissions: any): Promise<User> {
  const [updatedUser] = await db.update(users)
    .set({
