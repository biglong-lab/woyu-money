...
export async function getAiSettings(): Promise<AiSettings> {
  const rows = await db.select().from(aiSettings).where(eq(aiSettings.id, SETTINGS_ID)).limit(1)
...
/** 更新 AI 設定（upsert） */
export async function updateAiSettings(data: Partial<InsertAiSettings>): Promise<AiSettings> {
  // 確保記錄存在
  await getAiSettings()
