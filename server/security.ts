...
export const sanitizeInput = (input: unknown): unknown => {
  ...
  if (input && typeof input === "object") {
    ...
    for (const [key, value] of Object.entries(input)) {
      ...
      sanitized[String(sanitizedKey)] = sanitizeInput(value)
    }
    return sanitized
  }
...
export const checkPermission = (requiredPermission: string) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // 檢查用戶權限（menuPermissions 是 jsonb 型別）
      const userPermissions = (user.menuPermissions ?? {}) as Record<string, boolean>

      if (!userPermissions[requiredPermission]) {
        console.warn(
          `[Security] User ${user.id} attempted to access ${requiredPermission} without permission`
        )
...
export const secureQuery = async <T>(
  ...
  try {
    ...
    if (duration > 1000) {
      console.warn(
        `[Performance] Slow query in ${context}: ${duration}ms - User: ${user?.id || "system"}`
      )
    }
