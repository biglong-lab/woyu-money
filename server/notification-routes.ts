...
export function setupNotificationRoutes(app: Express) {
  ...
  app.get('/api/notifications', requireAuth, async (req, res) => {
    try {
      ...
      const notifications = await notificationSystem.getUserNotifications((req.user as any)!.id, limit);
  ...
  app.post('/api/notifications/read-all', requireAuth, async (req, res) => {
    try {
      const success = await notificationSystem.markAllAsRead((req.user as any)!.id);
      res.json({ success });
    } catch (error) {
  ...
  app.get('/api/notifications/unread-count', requireAuth, async (req, res) => {
    try {
      const count = await notificationSystem.getUnreadCount((req.user as any)!.id);
      res.json({ count });
    } catch (error) {
  ...
  app.get('/api/notification-settings', requireAuth, async (req, res) => {
    try {
      const settings = await notificationSystem.getNotificationSettings((req.user as any)!.id);
      res.json(settings);
    } catch (error) {
  ...
  app.put('/api/notification-settings', requireAuth, async (req, res) => {
    try {
      const success = await notificationSystem.updateNotificationSettings((req.user as any)!.id, req.body);
      res.json({ success });
    } catch (error) {
