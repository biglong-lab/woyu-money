...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      let validatedPriceTiers: Array<
        Omit<ReturnType<typeof insertRentalPriceTierSchema.parse>, "contractId">
      > = []
      if (priceTiers && priceTiers.length > 0) {
        validatedPriceTiers = priceTiers.map((tier: Record<string, unknown>) =>
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (year) {
      ...
      filteredPayments = allPayments.filter((p: RentalPayment) => {
        const dateToCheck = p.endDate || p.startDate
    ...
    if (contractId && contractId !== "all") {
      ...
      filteredPayments = filteredPayments.filter(
        (p: RentalPayment) => p.projectId === contractIdNum
      )
    }

    filteredPayments.sort((a: RentalPayment, b: RentalPayment) => {
      const dateA = new Date(a.endDate || a.startDate || 0)
      const dateB = new Date(b.endDate || b.startDate || 0)
      return dateA.getTime() - dateB.getTime()
    })

    const exportData: ExportRow[] = filteredPayments.map((payment: RentalPayment) => {
      const isPaid = payment.status === "paid"
      const baseRow: ExportRow = {
        期別: payment.itemName || "",
        合約名稱: payment.projectName || "",
        租金金額: parseFloat(payment.totalAmount) || 0,
        到期日: payment.endDate || payment.startDate || "",
        付款狀態: isPaid ? "已付款" : "未付款",
        付款日期: "",
        付款方式: "",
        備註: payment.notes || "",
      ...
      if (includeDetails === "true") {
        ...
        baseRow["合約ID"] = payment.projectId || ""
    ...
    const totalAmount = filteredPayments.reduce(
      (sum: number, p: RentalPayment) => sum + (parseFloat(p.totalAmount) || 0),
      0
    )
    const paidCount = filteredPayments.filter((p: RentalPayment) => p.status === "paid").length
    const unpaidCount = filteredPayments.length - paidCount
    const paidAmount = filteredPayments
      .filter((p: RentalPayment) => p.status === "paid")
      .reduce((sum: number, p: RentalPayment) => sum + (parseFloat(p.totalAmount) || 0), 0)
    const unpaidAmount = totalAmount - paidAmount
    const completionRate = totalAmount > 0 ? ((paidAmount / totalAmount) * 100).toFixed(1) : "0"

    const quarterlyStats = year
      ? [1, 2, 3, 4].map((q) => {
          const qPayments = filteredPayments.filter((p: RentalPayment) => {
            const dateToCheck = p.endDate || p.startDate
            if (!dateToCheck) return false
          ...
          return {
            ...
            paidCount: qPayments.filter((p: RentalPayment) => p.status === "paid").length,
            total: qTotal,
            paid: qPaid,
          }
        })
