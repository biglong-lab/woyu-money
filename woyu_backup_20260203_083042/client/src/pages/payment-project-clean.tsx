...
function PaymentProjectContent() {
  ...
  const { data: paymentItemsData } = useQuery({
    ...
    queryFn: () => {
      ...
      return apiRequest("GET", `/api/payment/items?${params}`);
    },
    staleTime: 2 * 60 * 1000,
  });
  ...
  const monthlyStats = useMemo(() => {
    ...
    const totalPayable = currentMonthItems.reduce((sum, item) => 
      sum + parseFloat(item.totalAmount || '0'), 0);
    
    const totalPaid = currentMonthItems.reduce((sum, item) => 
      sum + parseFloat(item.paidAmount || '0'), 0);
    
    const pendingItems = currentMonthItems.filter(item => item.status === 'pending').length;
    
    const overdueItems = currentMonthItems.filter(item => {
...
export default function PaymentProjectClean() {
