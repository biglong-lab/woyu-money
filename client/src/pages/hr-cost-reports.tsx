...
export default function HRCostReports() {
  ...
  const { data: yearReport, isLoading: isLoadingYear } = useQuery<HRYearReport>({
    ...
    queryFn: () =>
      apiRequest("GET", `/api/reports/hr-cost-report?year=${year}`) as Promise<HRYearReport>,
  })
  ...
  const { data: monthDetails = [], isLoading: isLoadingMonth } = useQuery<MonthDetail[]>({
    ...
    queryFn: () =>
      apiRequest("GET", `/api/reports/hr-cost-report/${year}/${month}`) as Promise<MonthDetail[]>,
  })
  ...
  const minMonth = trendData
    .filter((m) => m.totalCost > 0)
    .reduce<MonthlyBreakdown | null>(
      (min, m) => (!min || m.totalCost < min.totalCost ? m : min),
      null
    )
  const avgCost =
    trendData.length > 0
      ? trendData.reduce((sum, m) => sum + m.totalCost, 0) /
        trendData.filter((m) => m.totalCost > 0).length
      : 0
  const avgPerPerson =
    trendData.length > 0
      ? trendData.reduce(
          (sum, m) => sum + (m.employeeCount > 0 ? m.totalCost / m.employeeCount : 0),
          0
        ) / trendData.filter((m) => m.employeeCount > 0).length
      : 0
          ...
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="pt-4 text-center">
              ...
              <p className="text-lg font-bold text-blue-800">
                ${fmt(yearReport.yearTotal.salaryTotal)}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-orange-50 border-orange-200">
            <CardContent className="pt-4 text-center">
              ...
              <p className="text-lg font-bold text-orange-800">
                ${fmt(yearReport.yearTotal.insuranceTotal)}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-purple-50 border-purple-200">
            <CardContent className="pt-4 text-center">
              ...
              <p className="text-lg font-bold text-purple-800">
                ${fmt(yearReport.yearTotal.pensionTotal)}
              </p>
            </CardContent>
          </Card>
          <Card className="bg-green-50 border-green-200">
            <CardContent className="pt-4 text-center">
              ...
              <p className="text-lg font-bold text-green-800">
                ${fmt(yearReport.yearTotal.totalCost)}
              </p>
            </CardContent>
          </Card>
        </div>
                      ...
                      {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => {
                        ...
                        return (
                          <TableRow key={m} className={data ? "" : "text-gray-300"}>
                            ...
                            <TableCell className="text-right">
                              {data ? `$${fmt(data.salaryTotal)}` : "-"}
                            </TableCell>
                            <TableCell className="text-right">
                              {data ? `$${fmt(data.insuranceTotal)}` : "-"}
                            </TableCell>
                            <TableCell className="text-right">
                              {data ? `$${fmt(data.pensionTotal)}` : "-"}
                            </TableCell>
                      ...
                      {monthDetails.map((d, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="text-right text-orange-600">
                            $
                            {fmt(
                              d.employerLaborInsurance +
                                d.employerHealthInsurance +
                                d.employerEmploymentInsurance +
                                d.employerAccidentInsurance
                            )}
                          </TableCell>
                          <TableCell className="text-right text-purple-600">
                            ${fmt(d.employerPension)}
                          </TableCell>
                          ...
                          <TableCell className="text-center">
                            <div className="flex justify-center gap-1">
                              <Badge
                                variant={d.isPaid ? "default" : "destructive"}
                                className="text-xs"
                              >
                                {d.isPaid ? "薪已付" : "薪未付"}
                              </Badge>
                              <Badge
                                variant={d.insurancePaid ? "default" : "destructive"}
                                className="text-xs"
                              >
                  ...
                  return (
                    ...
                    >
                      <CardContent className="pt-3 pb-3 text-center">
                        ...
                        {data && <p className="text-xs text-gray-500">{data.employeeCount} 人</p>}
                        {change !== null && !isNaN(change) && (
                          <p
                            className={`text-xs mt-1 ${change >= 0 ? "text-red-600" : "text-green-600"}`}
                          >
                            {change >= 0 ? "+" : ""}
                            {change.toFixed(1)}%
                          </p>
