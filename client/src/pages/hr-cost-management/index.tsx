...
import type { Employee, MonthlyHrCost, EmployeeFormData, MonthTotal } from "./types"
...
export default function HRCostManagement() {
  ...
  const addEmployeeMutation = useMutation({
    mutationFn: (data: EmployeeSubmitData) => apiRequest("POST", "/api/hr/employees", data),
    onSuccess: () => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 mt-1">管理員工資料、計算勞健保費用、產生月度人事費</p>
        </div>
      ...
      {/* 概覽卡片 */}
      <OverviewCards activeCount={activeEmployees.length} monthTotal={monthTotal} />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="monthly" className="space-y-4">
          <MonthlyCostTable
            ...
            onUpdatePayStatus={(id, data) => updatePayStatusMutation.mutate({ id, data })}
          />
        </TabsContent>
      ...
      <EmployeeFormDialog
        ...
        isPending={addEmployeeMutation.isPending || updateEmployeeMutation.isPending}
      />
    </div>
  )
}
