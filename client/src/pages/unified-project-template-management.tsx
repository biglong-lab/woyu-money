...
export default function UnifiedProjectTemplateManagement() {
  ...
  const [editingTemplate, setEditingTemplate] = useState<
    ProjectCategoryTemplate | FixedCategorySubOption | null
  >(null)
  ...
  const { data: fixedTemplates = [], refetch: refetchFixedTemplates } = useQuery<
    FixedCategorySubOption[]
  >({
  ...
  const fixedTemplatesByCategory = fixedTemplates.reduce(
    (acc, template) => {
    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>選擇要管理模板的專案，所有模板都會綁定到此專案</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex gap-4 items-end">
            ...
            <Button onClick={handleCreateTemplate} disabled={!selectedProject}>
      ...
      {selectedProject && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>此專案的所有模板（固定項目與自訂分類）統一管理</CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="fixed" className="w-full">
              ...
              <TabsContent value="fixed" className="space-y-4">
                ...
                {Object.keys(fixedTemplatesByCategory).length > 0 ? (
                  <div className="space-y-4">
                    {Object.entries(fixedTemplatesByCategory).map(
                      ([fixedCategoryId, templates]) => {
                        const fixedCategory = fixedCategories.find(
                          (c) => c.id === parseInt(fixedCategoryId)
                        )
                                  ...
                                  <CardContent className="p-3">
                                    <div className="flex justify-between items-start mb-2">
                                      <h5 className="font-medium">
                                        {template.displayName || template.subOptionName}
                                      </h5>
                  ...
                  <div className="text-center py-8 text-gray-500">此專案尚無固定項目模板</div>
                )}
              </TabsContent>
              ...
              <TabsContent value="custom" className="space-y-4">
                ...
                {Object.keys(customTemplatesByCategory).length > 0 ? (
                  ...
                  <div className="text-center py-8 text-gray-500">此專案尚無自訂分類模板</div>
                )}
              </TabsContent>
            </Tabs>
          </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <div className="text-sm text-gray-600 space-y-2">
                  <p>
                    <strong>綁定關係：</strong> 專案 → 固定分類 → 帳號資訊
                  </p>
                  <p>
                    <strong>使用情境：</strong> 電話費088219194、948883776
                  </p>
                  <p>
                    <strong>資料表：</strong> fixed_category_sub_options
                  </p>
                  <p>
                    <strong>特點：</strong> 專案特定，確保單一性
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="text-sm text-gray-600 space-y-2">
                  <p>
                    <strong>綁定關係：</strong> 專案 → 自訂分類 → 帳號資訊
                  </p>
                  <p>
                    <strong>使用情境：</strong> 特殊分類的多個帳號
                  </p>
                  <p>
                    <strong>資料表：</strong> project_category_templates
                  </p>
                  <p>
                    <strong>特點：</strong> 彈性更高，支援複雜場景
                  </p>
                </div>
              </div>
            </div>
          ...
          <Form {...form}>
            <form className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="例：主線電話、088219194" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div>
                ...
                <Select
                  value={templateType}
                  onValueChange={(value: "custom" | "fixed") => setTemplateType(value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="電話號碼、電號、水號等帳號資訊" rows={3} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
