...
export default function GeneralPaymentManagement() {
  ...
  useEffect(() => {
    if (watchedFixedCategoryId && fixedCategories) {
      const selectedFixedCategory = fixedCategories.find(
        (cat) => cat.id === parseInt(watchedFixedCategoryId)
      )
  ...
  const statusCounts = useMemo(() => {
    ...
    return {
      ...
      overdue: generalItems.filter(
        (item) => item.status !== "paid" && new Date(item.startDate) < today
      ).length,
    ...
    return generalItems
      .filter((item) => {
        ...
        const matchesProject =
          selectedProject === "all" || item.projectId?.toString() === selectedProject
  ...
  const statistics: GeneralPaymentStatistics = useMemo(() => {
    ...
    const monthlyDue = currentMonthItems.reduce(
      (sum, item) => sum + parseFloat(item.totalAmount || "0"),
      0
    )
    const monthlyPaid = currentMonthItems.reduce(
      (sum, item) => sum + parseFloat(item.paidAmount || "0"),
      0
    )
    const overdueUnpaid = pastDueItems.reduce(
      (sum, item) =>
        sum + (parseFloat(item.totalAmount || "0") - parseFloat(item.paidAmount || "0")),
      0
    )
    ...
    const urgentItems = filteredItems.filter(
      (item) =>
        item.status !== "paid" &&
        new Date(item.startDate) <= urgentDate &&
    ...
    mutationFn: async (data: {
      ...
      const updateData = {
        ...
        fixedCategoryId:
          ...
          fixedCategories.find((c) => c.id === parseInt(editSelectedCategoryId))
            ? parseInt(editSelectedCategoryId)
            : null,
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-4 sm:px-0">
      ...
      <GeneralPaymentPaymentDialog
        ...
        onSubmit={
          handlePaymentSubmit as unknown as (data: FieldValues, receiptFile: File | null) => void
        }
