...
export default function PaymentScheduleOptimized() {
  ...
  const { data: integratedItems = [], isLoading } = useQuery<IntegratedPaymentItem[]>({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/payment/items/integrated?year=${year}&month=${month}`
      )
  ...
  const stats: ScheduleStats = {
    ...
    scheduledAmount: monthSchedules.reduce(
      (sum, s) => sum + parseFloat(s.scheduledAmount || "0"),
      0
    ),
    totalPending: integratedItems.reduce(
  ...
  const scheduleExecutionRate =
    stats.scheduledAmount > 0 ? (currentMonthPaid / stats.scheduledAmount) * 100 : 0
  ...
  const handleQuickPayment = (item: IntegratedPaymentItem, schedule?: Schedule) => {
    setLocation(
      `/payment-records?itemId=${item.id}&amount=${schedule?.scheduledAmount || item.pendingAmount}&date=${schedule?.scheduledDate || ""}`
    )
  }
    ...
    <div className="space-y-4 sm:space-y-6 p-3 sm:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4">
          ...
          <span
            className="text-sm sm:text-lg font-semibold min-w-[100px] sm:min-w-[120px] text-center"
