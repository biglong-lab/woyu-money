...
export function useRelatedPaymentItems(contract: ContractData | undefined) {
  const { data: paymentItems, isLoading } = useQuery<PaymentItem[]>({
    queryKey: ["/api/payment/items?includeFuture=true"],
    select: (data: unknown) => {
      if (!contract) return []
      ...
      return items.filter(
        (item) =>
          item.itemName?.includes(contract.contractName) || item.projectId === contract.projectId
      )
    },
    enabled: !!contract,
  })
...
export function useDocumentUpload(contractId: string | undefined) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (data: { file: File; description: string }) => {
      ...
      const response = await fetch(`/api/rental/contracts/${contractId}/documents`, {
        method: "POST",
        body: formData,
      })
...
export function useDocumentDelete(contractId: string | undefined) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (documentId: number) => {
      return apiRequest(`/api/rental/contracts/${contractId}/documents/${documentId}`, "DELETE")
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [`/api/rental/contracts/${contractId}/documents`],
...
export function useGeneratePayments(contractId: number) {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`/api/rental/contracts/${contractId}/generate-payments`, {
        method: "POST",
      })
      if (!response.ok) {
        const errorText = await response.text()
