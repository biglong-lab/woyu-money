...
export default function RentalManagementEnhanced() {
  ...
  const [priceTiers, setPriceTiers] = useState<PriceTier[]>([
    { yearStart: 1, yearEnd: 3, monthlyAmount: 0 },
  ])
  ...
  const { data: viewingContractDetails, isLoading: isLoadingContractDetails } =
    useQuery<RentalContractDetail>({
  ...
  const generatePaymentsMutation = useMutation({
    mutationFn: async (contractId: number) => {
      return apiRequest<GeneratePaymentsResult>(
        "POST",
  ...
  const smartAdjustMutation = useMutation({
    mutationFn: async (data: SmartAdjustData) => {
      return apiRequest<SmartAdjustResult>(
        "POST",
        `/api/rental/contracts/${selectedContract?.id}/smart-adjust`,
        data
      )
    },
  ...
  const previewAdjustment = async () => {
    ...
    try {
      const response = await apiRequest<AdjustmentPreview>(
        "POST",
        `/api/rental/contracts/${selectedContract?.id}/preview-adjustment`,
        formData
      )
  ...
  const handleEditContract = async (contract: RentalContractListItem) => {
    ...
    try {
      const detailResponse = await apiRequest<RentalContractDetail>(
        "GET",
        `/api/rental/contracts/${contract.id}`
      )
      ...
      form.reset({
        ...
        priceTiers:
          detailResponse.priceTiers && detailResponse.priceTiers.length > 0
            ...
            : [
                {
                  yearStart: 1,
                  yearEnd: 3,
                  monthlyAmount: parseFloat(detailResponse.baseAmount || "0"),
                },
              ],
      })
  ...
  return (
    <div className="space-y-4 md:space-y-6 p-4 md:p-6">
      ...
      <RentalPaymentDetailDialog
        ...
        viewingPayment={
          viewingPayment as unknown as Parameters<
            typeof RentalPaymentDetailDialog
          >[0]["viewingPayment"]
        }
      />
      {/* 刪除文件確認 */}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>此操作將永久刪除該租約文件、無法復原。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
