...
import {
  ...
  FilterStatsBar,
} from "@/components/monthly-payment-filter-panel"
...
export default function MonthlyPaymentManagement() {
  ...
  useEffect(() => {
    if (selectedFixedCategoryId) {
      const selectedCategory = fixedCategories.find(
        (cat) => cat.id === parseInt(selectedFixedCategoryId)
      )
      if (selectedCategory) {
  ...
  const handleCreateSubmit = async (data: CreateFormSubmitData) => {
    ...
    for (let i = 0; i < installments; i++) {
      ...
      const itemName =
        installments > 1 ? `${data.itemName} (第${i + 1}期/共${installments}期)` : data.itemName
