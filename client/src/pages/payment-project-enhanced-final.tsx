...
function PaymentProjectEnhancedFinal() {
  // 時間導航狀態 - 設置為實際數據存在的年月
  ...
  const paymentItems = paymentItemsData?.items || paymentItemsData || [];
  ...
  const handleEditItem = (item: PaymentItem) => {
    ...
    form.reset({
      ...
      paymentType: (item.paymentType === "installment" ? "installment" : "single"),
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle>
                {`${(projects as PaymentProject[]).find((p: PaymentProject) => p.id.toString() === selectedProject)?.projectName || '所有專案'} - ${filteredAndSortedItems.length} 個項目`}
              </CardTitle>
              <CardDescription>
                管理您的付款項目和追蹤付款狀態
              </CardDescription>
...
export default PaymentProjectEnhancedFinal;