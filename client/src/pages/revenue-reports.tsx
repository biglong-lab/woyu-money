...
export default function RevenueReports() {
  ...
  const processedYearlyData = (): ProcessedYearlyEntry[] => {
    const grouped = yearlyComparison.reduce((acc: Record<string, ProcessedYearlyEntry>, item) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">累計收款金額</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{revenueStats?.recordCount || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">平均每日收入</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{projectRevenues.length}</div>
            <p className="text-xs text-muted-foreground">有收款記錄的專案</p>
          </CardContent>
        </Card>
      </div>
      ...
      {revenueSources.length > 0 && (
        <div className="flex flex-wrap gap-2 items-center text-sm bg-blue-50 border border-blue-200 rounded-lg px-4 py-2">
          ...
          {revenueSources.map((s) => {
            const color =
              s.sourceKey === "pms-bridge"
                ? "bg-blue-100 text-blue-800"
                : s.sourceKey === "pm-bridge"
            ...
            return (
              <Badge key={s.sourceKey} variant="secondary" className={color}>
                {s.sourceName}
                &nbsp;·&nbsp;{s.recordCount.toLocaleString()} 筆 &nbsp;NT$
                {Math.round(s.totalRevenue).toLocaleString()}
              </Badge>
            )
          })}
        </div>
                ...
                {projectChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      ...
                      <Tooltip
                        formatter={(value: number) => [`NT$ ${value.toLocaleString()}`, "收入"]}
                      />
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {dailyTrend.length === 0 && (
                    <div className="text-center py-6 text-gray-500">尚無收款記錄</div>
                  )}
                </div>
              </CardContent>
            </Card>
              ...
              {processedYearlyData().length > 0 ? (
                <ResponsiveContainer width="100%" height={400}>
                  <LineChart data={processedYearlyData()}>
                    ...
                    <YAxis />
                    <Tooltip
                      formatter={(value: number) => [`NT$ ${value?.toLocaleString() || 0}`, "收入"]}
                    />
            ...
            <CardContent>
              <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-500">{project.recordCount} 筆記錄</div>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <Badge variant="secondary">
                        {((project.totalRevenue / (revenueStats?.totalRevenue || 1)) * 100).toFixed(
                          1
                        )}
                        %
                      </Badge>
                ...
                {projectRevenues.length === 0 && (
                  <div className="text-center py-6 text-gray-500">尚無專案收入數據</div>
                )}
              </div>
            </CardContent>
          </Card>
