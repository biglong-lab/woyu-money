  ...
  const { data: subcategoryStatuses = [] } = useQuery<SubcategoryStatus[]>({
    ...
    queryFn: () =>
      selectedParentCategory
        ? apiRequest(
            "GET",
            `/api/subcategory/status/${selectedParentCategory}${selectedProject ? `?projectId=${selectedProject}` : ""}`
          )
        : Promise.resolve([]),
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-4 bg-gray-50 rounded-lg p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                value={selectedParentCategory?.toString() || "none"}
                onValueChange={(value) =>
                  setSelectedParentCategory(value === "none" ? null : parseInt(value))
                }
              >
                <SelectTrigger className="w-48">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">
                        NT$ {parseInt(status.currentMonth.totalDue).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>已付金額：</span>
                      <span className="font-medium text-green-600">
                        NT$ {parseInt(status.currentMonth.totalPaid).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>未付項目：</span>
                      <Badge
                        variant={status.currentMonth.unpaidItems > 0 ? "destructive" : "secondary"}
                      >
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-red-600">
                        NT$ {parseInt(status.accumulated.totalUnpaid).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>逾期項目：</span>
                      <Badge
                        variant={status.accumulated.overdueItems > 0 ? "destructive" : "secondary"}
                      >
                  ...
                  <div className="bg-white rounded-lg p-3 border">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>總期數：</span>
                        <span className="font-medium">
                          {status.installments.totalInstallments} 期
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>已完成：</span>
                        <span className="font-medium text-green-600">
                          {status.installments.completedInstallments} 期
                        </span>
                      </div>
                      <Progress
                        value={
                          (status.installments.completedInstallments /
                            status.installments.totalInstallments) *
                          100
                        }
                        className="h-2"
                      />
                      {status.installments.nextDueDate && (
                        <div className="flex justify-between text-sm">
                          <span>下次到期：</span>
                          <span className="font-medium">
                            {format(new Date(status.installments.nextDueDate), "yyyy/MM/dd", {
                              locale: zhTW,
                            })}
                          </span>
                        </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="number" step="0.01" placeholder="請輸入付款金額" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
