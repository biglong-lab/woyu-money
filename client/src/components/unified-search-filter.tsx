...
export default function UnifiedSearchFilter({
  ...
  const [dueDateRange, setDueDateRange] = useState<
    "all" | "7days" | "30days" | "90days" | "overdue"
  >("all")
  const [isAdvancedOpen, setIsAdvancedOpen] = useState(false)
  const searchInputRef = useRef<HTMLInputElement>(null)

  // 全域 Ctrl+K / Cmd+K 聚焦搜尋（不在 input 中時也可觸發）
  useEffect(() => {
        ...
        <div className="relative flex-1">
          ...
          {search ? (
            <button
              type="button"
              onClick={() => setSearch("")}
              className="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded"
              title="清除搜尋（Esc）"
            >
              <X className="h-4 w-4" />
            </button>
        ...
        <div className="flex gap-2">
          <Popover open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className="relative flex-1 sm:flex-none min-h-[40px]"
                  ...
                  <div className="flex flex-wrap gap-1">
                    {priorities.map((priority) => (
                      <Badge
                        key={priority.value}
                        variant={
                          selectedPriorities.includes(priority.value) ? "default" : "outline"
                        }
                        className="cursor-pointer"
