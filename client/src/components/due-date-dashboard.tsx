...
export default function DueDateDashboard({
  ...
  const ItemCard = ({ item, urgency }: { item: PaymentItem; urgency: 'overdue' | 'urgent' | 'upcoming' | 'normal' }) => {
    ...
    return (
      <div
        className={`p-3 rounded-lg ${urgencyStyles[urgency]} cursor-pointer hover:shadow-md transition-shadow`}
    ...
    <div className={`space-y-4 ${className}`}>
      <div className="grid grid-cols-4 gap-4">
        <Card className={stats.overdueCount > 0 ? 'border-red-200 bg-red-50' : ''} data-testid="card-overdue-summary">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">逾期項目</p>
                <p className="text-2xl font-bold text-red-600">{stats.overdueCount}</p>
                <p className="text-xs text-red-500">${stats.overdueAmount.toLocaleString()}</p>
              </div>
              <AlertTriangle className={`h-8 w-8 ${stats.overdueCount > 0 ? 'text-red-500' : 'text-gray-300'}`} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className={stats.urgentCount > 0 ? 'border-orange-200 bg-orange-50' : ''} data-testid="card-urgent-summary">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">7天內到期</p>
                <p className="text-2xl font-bold text-orange-600">{stats.urgentCount}</p>
                <p className="text-xs text-orange-500">${stats.urgentAmount.toLocaleString()}</p>
              </div>
              <Clock className={`h-8 w-8 ${stats.urgentCount > 0 ? 'text-orange-500' : 'text-gray-300'}`} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card data-testid="card-upcoming-summary">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">30天內到期</p>
        ...
        <Card data-testid="card-total-pending">
          <CardContent className="pt-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">待付總額</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        {categorizedItems.overdue.length === 0 && categorizedItems.urgent.length === 0 && categorizedItems.upcoming.length === 0 && (
          <Card className="col-span-3 border-green-200 bg-green-50" data-testid="card-no-urgent">
            <CardContent className="pt-6 text-center">
              <CheckCircle className="h-12 w-12 text-green-500 mx-auto mb-2" />
              <h3 className="text-lg font-medium text-green-700">太棒了！</h3>
              <p className="text-sm text-green-600">目前沒有緊急需要處理的付款項目</p>
            </CardContent>
          </Card>
        )}
      </div>
