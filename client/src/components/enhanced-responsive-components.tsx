...
export function ResponsiveButtonGroup({
  ...
  return (
    <div className={cn(orientationClasses[orientation], "gap-2 sm:gap-3", className)}>
      ...
      <CardHeader className="cursor-pointer select-none pb-3" onClick={() => setIsOpen(!isOpen)}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            {badge && (
              <Badge variant="outline" className="flex-shrink-0 text-xs px-2 py-1">
                {badge}
              </Badge>
            )}
          </div>
          ...
          <thead>
            <tr className="border-b border-gray-200">
              {columns.map((column) => (
                <th
                  key={column.key}
                  className={cn("text-left py-3 px-4 font-medium text-gray-900", column.width)}
                >
                  {column.label}
                </th>
              ))}
...
export function StatusIndicator({
  ...
  return (
    ...
    >
      ...
      {description && <span className="opacity-75">• {description}</span>}
    </div>
  )
}
    ...
    <div className={cn("w-full", className)}>
      ...
      <div className={cn("md:block", open ? "block" : "hidden")}>
        <Card className="transition-all duration-300">
          ...
          <CardContent className="md:pt-0">{children}</CardContent>
        </Card>
      </div>
    </div>
  )
