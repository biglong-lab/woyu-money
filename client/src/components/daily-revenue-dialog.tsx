...
export function DailyRevenueDialog({
  ...
  const form = useForm<DailyRevenueFormData>({
    ...
    defaultValues: {
      ...
      date: revenue?.date || localDateISO(),
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: mode === "edit" ? "收款記錄已成功更新。" : "新收款記錄已成功新增。",
      })
    },
    onError: (error: Error) => {
      toast({
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === "edit" ? "修改收款記錄資訊" : "記錄專案的每日收款金額"}
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="收款說明或備註" rows={3} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="space-y-2">
              ...
              />
              {selectedFile && <p className="text-sm text-gray-600">已選擇：{selectedFile.name}</p>}
            </div>
