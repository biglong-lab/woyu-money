...
export default function FinancialHealthDashboard({
  ...
  const metrics = useMemo(() => {
    ...
    const overdueRate =
      pendingItems.length > 0 ? (overdueItems.length / pendingItems.length) * 100 : 0
  ...
  const statusDistribution = useMemo(
    () =>
      [
        ...
        {
          name: "待付款",
          value: metrics.pendingItems - metrics.overdueItems,
  ...
  const healthMetrics: HealthMetric[] = [
    {
      ...
      target: 80,
      status:
        metrics.completionRate >= 80 ? "good" : metrics.completionRate >= 50 ? "warning" : "danger",
      icon: CheckCircle,
    ...
    },
    {
      ...
      status:
        metrics.paymentProgress >= 70
          ? "good"
          : metrics.paymentProgress >= 40
            ? "warning"
            : "danger",
      icon: Target,
      description: `${formatNT(metrics.totalPaid)} / ${formatNT(metrics.totalPlanned)}`,
    },
    {
      label: "高優先待處理",
      value: metrics.highPriorityPending,
      status:
        metrics.highPriorityPending === 0
          ? "good"
          : metrics.highPriorityPending <= 3
            ? "warning"
            : "danger",
      icon: Clock,
            ...
            <div className="flex items-center justify-center py-2 sm:py-4">
              <div className="relative w-24 h-24 sm:w-32 sm:h-32">
                ...
                <svg viewBox="0 0 128 128" className="w-full h-full">
                  ...
                  <circle
                    className={`${
                      getScoreStatus(healthScore) === "good"
                        ? "text-green-500"
                        : getScoreStatus(healthScore) === "warning"
                          ? "text-yellow-500"
                          : "text-red-500"
                    }`}
                    strokeWidth="10"
                ...
                <div className="flex flex-col sm:flex-row sm:items-baseline sm:gap-2">
                  ...
                  {metric.target !== undefined && (
                    <span className="text-[10px] sm:text-xs text-gray-500">
                      目標:{" "}
                      {metric.label === "逾期率" ? `≤${metric.target}%` : `≥${metric.target}%`}
                    </span>
                  )}
                </div>
                {metric.target !== undefined && (
