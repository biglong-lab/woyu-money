      ...
      className={`transition-all hover:shadow-md border-l-4 ${
        item.paymentType === "installment"
          ...
          : item.status === "partial"
            ...
            : new Date(
                  item.paymentType === "single" ? item.startDate : item.endDate || item.startDate
                ) < new Date()
          ...
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1 min-w-0 space-y-3" onClick={() => onItemClick(item)}>
              ...
              {item.status === "partial" && (
                <div className="space-y-1">
                  <div className="flex justify-between text-xs text-gray-600">
                    ...
                    <span>
                      {Math.round(
                        (parseFloat(item.paidAmount || "0") / parseFloat(item.totalAmount)) * 100
                      )}
                      %
                    </span>
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
                {item.categoryName && (
                  <span className="bg-gray-100 px-2 py-1 rounded">分類: {item.categoryName}</span>
                )}
                {item.projectName && (
                  <span className="bg-blue-100 px-2 py-1 rounded">專案: {item.projectName}</span>
                )}
    ...
    <>
      ...
      {activeTab === "items" ? (
        <ResponsiveLayout maxWidth="7xl" padding="md">
          {/* 專案選擇和統計概覽 */}
          <ResponsiveGrid cols={{ default: 1, md: 4 }} gap="md" className="mb-6">
          ...
          <div className="space-y-6">
            {itemsLoading ? (
              ...
              <>
                {/* 未付款項目 */}
                {(() => {
                  const unpaidItems = filteredAndSortedItems.filter(
                        ...
                        <CardContent className="p-4">
                          ...
                          {showPaidItems && (
                            <div className="mt-4 space-y-3">
                              {paidItems.map((item) => (
                                <PaidItemCard key={item.id} item={item} onItemClick={onItemClick} />
                              ))}
                            </div>
                          )}
                        </CardContent>
              ...
              <div className="text-center py-12">
                <div className="text-gray-500 mb-4">
                  {searchTerm || selectedStatus !== "all" || selectedProject !== "all" ? (
                    ...
                    "此專案還沒有付款項目，點擊上方按鈕新增第一個項目"
                  ) : (
                    "還沒有專案付款項目，點擊上方按鈕新增第一個項目"
                  )}
                </div>
              </div>
            )}
