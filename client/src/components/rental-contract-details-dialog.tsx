        ...
        {isLoading ? (
          ...
          <Tabs value={contractDetailsTab} onValueChange={onContractDetailsTabChange}>
            ...
            <TabsContent value="details" className="space-y-6">
              <ContractDetailsTab details={viewingContractDetails} projects={projects} />
            </TabsContent>
          ...
          <div className="text-center text-gray-500">無法載入租約詳情</div>
        )}
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              <strong>專案：</strong>
              {projects?.find((p) => p.id === details.projectId)?.projectName || "未知"}
            </div>
            <div>
              <strong>合約期間：</strong>
              {details.startDate
                ? new Date(details.startDate).toLocaleDateString("zh-TW")
                : "未設定"}{" "}
              - {details.endDate ? new Date(details.endDate).toLocaleDateString("zh-TW") : "未設定"}
            </div>
            <div>
              <strong>總年數：</strong>
              {details.totalYears || 0}年
            </div>
            <div>
              <strong>基礎金額：</strong>NT$
              {parseFloat(String(details.baseAmount || 0)).toLocaleString()}
            </div>
            {details.hasBufferPeriod && (
              <div className="bg-blue-50 p-3 rounded">
                <strong>緩衝期設定：</strong>
                <div className="text-sm mt-1">
                  緩衝期月數：{details.bufferMonths}個月
                  <br />
            ...
            {details.notes && (
              <div>
                <strong>備註：</strong>
                <div className="bg-gray-50 p-2 rounded text-sm mt-1">{details.notes}</div>
              </div>
            )}
          </CardContent>
        </Card>
                ...
                <TableBody>
                  {details.priceTiers.map((tier: RentalPriceTier, index: number) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        NT${parseFloat(String(tier.monthlyAmount || 0)).toLocaleString()}
                      </TableCell>
                      <TableCell>
                        NT${(parseFloat(String(tier.monthlyAmount || 0)) * 12).toLocaleString()}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <CardContent>
          {(() => {
            if (!details.startDate || !details.endDate) {
              return <div className="text-gray-500">合約日期資訊不完整，無法計算進度</div>
            }
            ...
            if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
              return <div className="text-gray-500">合約日期格式錯誤，無法計算進度</div>
            }
            ...
            const progress =
              totalDays > 0 ? Math.max(0, Math.min(100, (passedDays / totalDays) * 100)) : 0
            ...
            return (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  ...
                  <div>
                    <strong>剩餘天數：</strong>
                    {Math.max(0, totalDays - passedDays)}天
                  </div>
                </div>
              </div>
            )
          ...
          <div className="grid grid-cols-12 gap-2 mb-4">
            {Array.from({ length: 12 }, (_, index) => {
              ...
              const monthlyPayments = contractPaymentItems.filter((item) => {
                ...
                return (
                  itemDate.getFullYear() === monthlyPaymentYear && itemDate.getMonth() + 1 === month
                )
              })
              ...
              if (monthlyPayments.length > 0) {
                const totalAmount = monthlyPayments.reduce(
                  (sum: number, item: ContractPaymentItem) =>
                    sum + (parseFloat(item.totalAmount) || 0),
                  0
                )
                const paidAmount = monthlyPayments.reduce(
                  (sum: number, item: ContractPaymentItem) =>
...
function YearlyStatsSummary({
  ...
  const totalAmount = yearPayments.reduce(
    (sum: number, item: ContractPaymentItem) => sum + (parseFloat(item.totalAmount) || 0),
    0
  )
  const paidAmount = yearPayments.reduce(
    (sum: number, item: ContractPaymentItem) => sum + (parseFloat(item.paidAmount || "0") || 0),
    0
  )
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <Card>
        <CardContent className="p-4">
          <div className="text-2xl font-bold text-blue-600">NT${totalAmount.toLocaleString()}</div>
          <div className="text-sm text-gray-600">年度總金額</div>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="p-4">
          <div className="text-2xl font-bold text-green-600">NT${paidAmount.toLocaleString()}</div>
          <div className="text-sm text-gray-600">已付金額</div>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="p-4">
          <div className="text-2xl font-bold text-red-600">NT${unpaidAmount.toLocaleString()}</div>
...
function ContractPaymentTable({
  ...
  const yearPayments = contractPaymentItems
    .filter((item) => {
      if (!item.startDate) return false
      const itemDate = new Date(item.startDate)
      return itemDate.getFullYear() === monthlyPaymentYear
                ...
                {yearPayments.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} data-testid={`payment-row-${item.id}`}>
                      <TableCell className="font-medium">{itemDate.getMonth() + 1}月</TableCell>
                      <TableCell>{itemDate.toLocaleDateString("zh-TW")}</TableCell>
                      <TableCell className="text-right">NT${total.toLocaleString()}</TableCell>
                      ...
                      <TableCell>
                        <Link href={`/general-payment-management?itemId=${item.id}`}>
                          <Button
                            variant="outline"
                            size="sm"
                            data-testid={`btn-view-payment-${item.id}`}
                          >
